...
main (int argc, char **argv)
{
  ...
  try {
    boost::program_options::options_description desc (
        "kurento-media-server usage");

    desc.add_options () ("help,h", "Display this help message") (
        "version,v", "Display the version number") (
        "list,l", "Lists all available factories") ("modules-path,p",
        boost::program_options::value<std::string> (&modulesPath),
        "Colon-separated path(s) where Kurento modules can be found") (
        "conf-file,f",
        boost::program_options::value<std::string> (&confFile)->default_value (
            DEFAULT_CONFIG_FILE),
        "Configuration file location") ("logs-path,d",
        boost::program_options::value<std::string> (&logsPath),
        ...
        "Path where modules config files can be found") ("log-file-size,s",
        boost::program_options::value<int> (&fileSize)->default_value (
            DEFAULT_LOG_FILE_SIZE),
        "Maximum file size for log files, in MB") ("number-log-files,n",
        boost::program_options::value<int> (&fileNumber)
            ->default_value (DEFAULT_LOG_FILE_COUNT),
        ...
        boost::program_options::parse_environment (desc,
            [&desc] (std::string input) -> std::string {
              ...
              if (input.find (ENV_PREFIX) == 0) {
                ...
                std::transform (
                    aux.begin (), aux.end (), aux.begin (), [] (int c) -> int {
  ...
  if (killResourceLimit) {
    GST_INFO (
        "Using above %.2f%% of system limits will kill the server when no objects are alive",
