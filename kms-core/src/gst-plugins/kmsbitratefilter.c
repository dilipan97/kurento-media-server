...
kms_bitrate_calc_data_clear (KmsBitrateCalcData *data)
{
  ...
  g_queue_free_full (
      data->pts_queue, (GDestroyNotify)kms_utils_destroy_guint64);
...
static GstFlowReturn
kms_bitrate_filter_prepare_buf (GstBaseTransform *trans,
    GstBuffer *input,
    GstBuffer **buf)
{
...
kms_bitrate_filter_class_init (KmsBitrateFilterClass *klass)
{
  ...
  gst_element_class_set_details_simple (gstelement_class, "BitrateFilter",
      "Generic", "Pass data without modification, managing bitrate caps.",
...
kms_bitrate_filter_plugin_init (GstPlugin *plugin)
{
  return gst_element_register (
      plugin, PLUGIN_NAME, GST_RANK_NONE, KMS_TYPE_BITRATE_FILTER);
}
