...
public:
  ...
  {
    std::string desc = "_"
        + (sinkDescription.empty () ? KMS_DEFAULT_MEDIA_DESCRIPTION
                                    : sinkDescription);
  ...
  void
  setSourcePadName (gchar *padName)
  {
  ...
  std::shared_ptr<ElementConnectionData>
  toInterface ()
  {
    std::shared_ptr<ElementConnectionData> iface (
        new ElementConnectionData (source.lock (), sink.lock (), type,
            sourceDescription, sinkDescription));

    if (!iface->getSink ()) {
      throw KurentoException (
          MEDIA_OBJECT_NOT_FOUND, "Reference to sink is null");
    }

    if (!iface->getSource ()) {
      throw KurentoException (
          MEDIA_OBJECT_NOT_FOUND, "Reference to source is null");
    }
  ...
  do {
    ...
    if (GST_PAD_IS_SRC (pad)) {
      std::unique_lock<std::recursive_timed_mutex> lock (
          self->sinksMutex, std::defer_lock);
      std::shared_ptr<MediaType> type;
        ...
        for (auto it : connections) {
          if (g_strcmp0 (GST_OBJECT_NAME (pad), it->getSourcePadName ()) == 0) {
            std::unique_lock<std::recursive_timed_mutex> sinkLock (
                std::dynamic_pointer_cast<MediaElementImpl> (it->getSink ())
                    ->sourcesMutex,
                std::defer_lock);
      ...
      std::unique_lock<std::recursive_timed_mutex> lock (
          self->sourcesMutex, std::defer_lock);
      std::shared_ptr<MediaType> type;
      ...
      if (g_str_has_prefix (GST_OBJECT_NAME (pad), "sink_audio_")) {
        type = std::make_shared<MediaType> (MediaType::AUDIO);
        description =
            std::string (GST_OBJECT_NAME (pad) + sizeof ("sink_audio"));
      } else if (g_str_has_prefix (GST_OBJECT_NAME (pad), "sink_video_")) {
        type = std::make_shared<MediaType> (MediaType::VIDEO);
        description =
            std::string (GST_OBJECT_NAME (pad) + sizeof ("sink_video"));
      } else {
        type = std::make_shared<MediaType> (MediaType::DATA);
        description =
            std::string (GST_OBJECT_NAME (pad) + sizeof ("sink_data"));
      }
      ...
      try {
        ...
        if (source) {
          if (g_strcmp0 (
                  GST_OBJECT_NAME (pad), sourceData->getSinkPadName ().c_str ())
              == 0) {
            std::unique_lock<std::recursive_timed_mutex> sourceLock (
                source->sinksMutex, std::defer_lock);
...
void
MediaElementImpl::mediaFlowOutStateChange (gboolean isFlowing,
    gchar *padName,
...
void
MediaElementImpl::mediaFlowInStateChange (gboolean isFlowing,
    gchar *padName,
    KmsElementPadType type)
{
  std::shared_ptr<MediaFlowState> state;
  if (isFlowing) {
    GST_DEBUG_OBJECT (element,
        "MediaFlowInStateChange: FLOWING"
        ", pad: '%s', type: '%s'",
        padName, padTypeToString (type).c_str ());
    state = std::make_shared<MediaFlowState> (MediaFlowState::FLOWING);
  } else {
    GST_DEBUG_OBJECT (element,
        "MediaFlowInStateChange: NOT FLOWING"
        ", pad: '%s', type: '%s'",
        padName, padTypeToString (type).c_str ());
    state = std::make_shared<MediaFlowState> (MediaFlowState::NOT_FLOWING);
  }
    ...
    KmsElementPadType type)
{
  ...
  if (isTranscoding) {
    GST_INFO_OBJECT (element,
        "MediaTranscodingStateChange: TRANSCODING"
        ", bin: '%s', type: '%s'",
        binName, padTypeToString (type).c_str ());
    state = std::make_shared<MediaTranscodingState> (
        MediaTranscodingState::TRANSCODING);
  } else {
    GST_INFO_OBJECT (element,
        "MediaTranscodingStateChange: NOT TRANSCODING"
        ", bin: '%s', type: '%s'",
        binName, padTypeToString (type).c_str ());
    state = std::make_shared<MediaTranscodingState> (
  ...
  try {
    ...
    GST_WARNING_OBJECT (
        element, "Cannot emit event: MediaTranscodingStateChange");
  }
...
void
MediaElementImpl::disconnectAll ()
{
  while (!MediaElementImpl::getSinkConnections ().empty ()) {
    std::unique_lock<std::recursive_timed_mutex> sinkLock (
        sinksMutex, std::defer_lock);
    ...
    for (std::shared_ptr<ElementConnectionData> connData :
      ...
      auto sinkImpl =
          std::dynamic_pointer_cast<MediaElementImpl> (connData->getSink ());
      std::unique_lock<std::recursive_timed_mutex> sinkLock (
          sinkImpl->sourcesMutex, std::defer_lock);
  ...
  while (!MediaElementImpl::getSourceConnections ().empty ()) {
    ...
    for (std::shared_ptr<ElementConnectionData> connData :
      ...
      auto sourceImpl =
          std::dynamic_pointer_cast<MediaElementImpl> (connData->getSource ());
      std::unique_lock<std::recursive_timed_mutex> sourceLock (
          sourceImpl->sinksMutex, std::defer_lock);
...
std::vector<std::shared_ptr<ElementConnectionData>>
MediaElementImpl::getSourceConnections (std::shared_ptr<MediaType> mediaType)
{
  std::unique_lock<std::recursive_timed_mutex> lock (sourcesMutex);
  std::vector<std::shared_ptr<ElementConnectionData>> ret;
...
std::vector<std::shared_ptr<ElementConnectionData>>
MediaElementImpl::getSinkConnections (std::shared_ptr<MediaType> mediaType)
{
  std::unique_lock<std::recursive_timed_mutex> lock (sinksMutex);
  std::vector<std::shared_ptr<ElementConnectionData>> ret;
...
MediaElementImpl::connect (std::shared_ptr<MediaElement> sink)
{
  ...
  connect (
      sink, std::make_shared<MediaType> (MediaType::AUDIO), DEFAULT, DEFAULT);
  connect (
      sink, std::make_shared<MediaType> (MediaType::VIDEO), DEFAULT, DEFAULT);
  connect (
      sink, std::make_shared<MediaType> (MediaType::DATA), DEFAULT, DEFAULT);
}
...
void
MediaElementImpl::prepareSinkConnection (std::shared_ptr<MediaElement> src,
    std::shared_ptr<MediaType> mediaType,
    const std::string &sourceMediaDescription,
    ...
    const std::string &sinkMediaDescription)
{
  ...
  if (sinkImpl->getMediaPipeline ()->getId ()
      != getMediaPipeline ()->getId ()) {
    throw KurentoException (
        CONNECT_ERROR, "Media elements do not share pipeline");
  }

  std::unique_lock<std::recursive_timed_mutex> lock (sinksMutex);
  std::unique_lock<std::recursive_timed_mutex> sinkLock (
      sinkImpl->sourcesMutex);
  std::vector<std::shared_ptr<ElementConnectionData>> connections;
  std::shared_ptr<ElementConnectionDataInternal> connectionData (
      new ElementConnectionDataInternal (
          std::dynamic_pointer_cast<MediaElement> (shared_from_this ()), sink,
          mediaType, sourceMediaDescription, sinkMediaDescription));
  ...
  if (!connections.empty ()) {
    ...
    connection->getSource ()->disconnect (connection->getSink (), mediaType,
        sourceMediaDescription, connection->getSinkDescription ());
  }
  ...
  g_signal_emit_by_name (getGstreamerElement (), "request-new-pad", type,
      sourceMediaDescription.c_str (), GST_PAD_SRC, &padName, NULL);

  if (padName == nullptr) {
    throw KurentoException (CONNECT_ERROR,
        "Element: '" + getName () + "'does note provide a connection for "
            + mediaType->getString () + "-" + sourceMediaDescription);
  }
  ...
  try {
    ElementConnected event (shared_from_this (), ElementConnected::getName (),
        sink, mediaType, sourceMediaDescription, sinkMediaDescription);
...
void
MediaElementImpl::performConnection (
    std::shared_ptr<ElementConnectionDataInternal> data)
{
...
void
MediaElementImpl::disconnect (std::shared_ptr<MediaElement> sink)
{
...
void
MediaElementImpl::setAudioFormat (std::shared_ptr<AudioCaps> caps)
{
...
void
MediaElementImpl::setVideoFormat (std::shared_ptr<VideoCaps> caps)
{
...
std::string
MediaElementImpl::getGstreamerDot (std::shared_ptr<GstreamerDotDetails> details)
{
  return generateDotGraph (GST_BIN (element), details);
}

std::string
MediaElementImpl::getGstreamerDot ()
{
  return generateDotGraph (GST_BIN (element),
      std::make_shared<GstreamerDotDetails> (
          GstreamerDotDetails::SHOW_VERBOSE));
}

void
MediaElementImpl::setOutputBitrate (int bitrate)
{
...
int
MediaElementImpl::getMinOuputBitrate ()
{
  gint bitrate;
...
int
MediaElementImpl::getMinOutputBitrate ()
{
  gint bitrate;
...
void
MediaElementImpl::setMinOuputBitrate (int minOuputBitrate)
{
  GST_WARNING ("minOuputBitrate property is deprecated, use property "
               "minOutputBitrate");
  g_object_set (G_OBJECT (element), MIN_OUTPUT_BITRATE, minOuputBitrate, NULL);
}

void
MediaElementImpl::setMinOutputBitrate (int minOutputBitrate)
{
  g_object_set (G_OBJECT (element), MIN_OUTPUT_BITRATE, minOutputBitrate, NULL);
}

int
MediaElementImpl::getMaxOuputBitrate ()
{
  gint bitrate;
...
int
MediaElementImpl::getMaxOutputBitrate ()
{
  gint bitrate;
...
void
MediaElementImpl::setMaxOuputBitrate (int maxOuputBitrate)
{
  GST_WARNING ("maxOuputBitrate property is deprecated, use property "
               "maxOutputBitrate");
  g_object_set (G_OBJECT (element), MAX_OUTPUT_BITRATE, maxOuputBitrate, NULL);
}

void
MediaElementImpl::setMaxOutputBitrate (int maxOutputBitrate)
{
  g_object_set (G_OBJECT (element), MAX_OUTPUT_BITRATE, maxOutputBitrate, NULL);
}
    ...
    int64_t timestampMillis)
{
  ...
  if (!GST_VALUE_HOLDS_STRUCTURE (value)) {
    ...
    GST_WARNING (
        "Unexpected field type (%s) = %s", KMS_MEDIA_ELEMENT_FIELD, str_val);
    g_free (str_val);
...
bool
MediaElementImpl::isMediaFlowingIn (std::shared_ptr<MediaType> mediaType)
{
...
bool
MediaElementImpl::isMediaFlowingOut (std::shared_ptr<MediaType> mediaType)
{
...
bool
MediaElementImpl::isMediaTranscoding (std::shared_ptr<MediaType> mediaType)
{
