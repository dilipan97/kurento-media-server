...
bus_msg (GstBus *bus, GstMessage *msg, gpointer pipe)
{
  ...
  case GST_MESSAGE_ERROR: {
    ...
    GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS (
        GST_BIN (pipe), GST_DEBUG_GRAPH_SHOW_ALL, "error");
  ...
  case GST_MESSAGE_WARNING: {
    ...
    GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS (
        GST_BIN (pipe), GST_DEBUG_GRAPH_SHOW_ALL, "warning");
    break;
  }
  default:
    break;
...
static void
on_ice_candidate (GstElement *self,
    gchar *sess_id,
    ...
    OnIceCandidateData *data)
{
  ...
  g_signal_emit_by_name (
      data->peer, "add-ice-candidate", data->peer_sess_id, candidate, &ret);
  fail_unless (ret);
}
...
static void
fakesink_hand_off (GstElement *fakesink,
    GstBuffer *buf,
    GstPad *pad,
    gpointer data)
{
...
static void
receiver_1_fakesink_hand_off (GstElement *fakesink,
    GstBuffer *buf,
    GstPad *pad,
    gpointer data)
{
  ...
  if (GPOINTER_TO_INT (g_object_get_qdata (
          G_OBJECT (pipeline), answerer_receives_quark ()))) {
    g_idle_add (quit_main_loop_idle, hod->loop);
  } else {
    g_object_set_qdata (
        G_OBJECT (pipeline), offerer_receives_quark (), GINT_TO_POINTER (TRUE));
  }
...
static void
receiver_2_fakesink_hand_off (GstElement *fakesink,
    GstBuffer *buf,
    GstPad *pad,
    gpointer data)
{
  ...
  if (GPOINTER_TO_INT (g_object_get_qdata (
          G_OBJECT (pipeline), offerer_receives_quark ()))) {
    g_idle_add (quit_main_loop_idle, hod->loop);
  } else {
...
static void
connect_sink_on_srcpad_added (GstElement *element,
    GstPad *pad,
    gpointer user_data)
{
  GstElement *sink;
...
kms_element_request_srcpad (GstElement *src, KmsElementPadType pad_type)
{
  ...
  g_signal_emit_by_name (
      src, "request-new-pad", pad_type, NULL, GST_PAD_SRC, &padname);
  if (padname == NULL) {
    return FALSE;
  }
...
static void
connect_sink_async (GstElement *webrtcendpoint,
    GstElement *src,
    GstElement *enc,
    GstElement *caps,
    GstElement *pipe,
    const gchar *pad_prefix)
{
  ...
  data->id = g_signal_connect_data (webrtcendpoint, "pad-added",
      G_CALLBACK (connect_sink), data, (GClosureNotify)kms_connect_data_destroy,
      0);
}

static void
test_video_sendonly (const gchar *video_enc_name,
    GstStaticCaps expected_caps,
    gchar *codec,
    gboolean bundle,
    gboolean check_request_local_key_frame,
    gboolean gather_asap,
    const gchar *pem)
{
  GArray *codecs_array;
  ...
  connect_sink_async (
      sender, videotestsrc, video_enc, NULL, pipeline, SINK_VIDEO_STREAM);
  ...
  mark_point ();
  g_signal_emit_by_name (
      receiver, "process-offer", receiver_sess_id, offer, &answer);
  ...
  mark_point ();
  g_signal_emit_by_name (
      sender, "process-answer", sender_sess_id, answer, &answer_ok);
  ...
  g_signal_connect (
      receiver, "pad-added", G_CALLBACK (connect_sink_on_srcpad_added), NULL);
  fail_unless (
      kms_element_request_srcpad (receiver, KMS_ELEMENT_PAD_TYPE_VIDEO));
  ...
  GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS (
      GST_BIN (pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "test_sendonly_end");
    ...
    gboolean rtcp_mux)
{
  ...
  mark_point ();
  g_signal_emit_by_name (
      answerer, "process-offer", answerer_sess_id, offer, &answer);
  fail_unless (answer != NULL);
  GST_DEBUG ("Answer:\n%s", (sdp_str = gst_sdp_message_as_text (answer)));
  g_free (sdp_str);
  sdp_str = NULL;
  ...
  mark_point ();
  g_signal_emit_by_name (
      offerer, "process-answer", offerer_sess_id, answer, &answer_ok);
  fail_unless (answer_ok);
  gst_sdp_message_free (offer);
  gst_sdp_message_free (answer);
  ...
  gst_bin_add_many (
      GST_BIN (pipeline), fakesink_offerer, fakesink_answerer, NULL);

  g_object_set_qdata (
      G_OBJECT (offerer), video_sink_quark (), fakesink_offerer);
  g_signal_connect (
      offerer, "pad-added", G_CALLBACK (connect_sink_on_srcpad_added), NULL);
  fail_unless (
      kms_element_request_srcpad (offerer, KMS_ELEMENT_PAD_TYPE_VIDEO));

  g_object_set_qdata (
      G_OBJECT (answerer), video_sink_quark (), fakesink_answerer);
  g_signal_connect (
      answerer, "pad-added", G_CALLBACK (connect_sink_on_srcpad_added), NULL);
  fail_unless (
  ...
  GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS (
      GST_BIN (pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "test_sendrecv_end");
    ...
    gboolean bundle)
{
  ...
  g_object_set_qdata (
      G_OBJECT (offerer), audio_sink_quark (), fakesink_offerer);
  g_signal_connect (
      offerer, "pad-added", G_CALLBACK (connect_sink_on_srcpad_added), NULL);
  fail_unless (
      kms_element_request_srcpad (offerer, KMS_ELEMENT_PAD_TYPE_AUDIO));

  g_object_set_qdata (
      G_OBJECT (answerer), audio_sink_quark (), fakesink_answerer);
  g_signal_connect (
      answerer, "pad-added", G_CALLBACK (connect_sink_on_srcpad_added), NULL);
  fail_unless (
  ...
  GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS (
      GST_BIN (pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "test_audio_sendrecv_end");
...
{
  return GPOINTER_TO_INT (g_object_get_qdata (
             G_OBJECT (pipeline), offerer_receives_audio_quark ()))
      && GPOINTER_TO_INT (g_object_get_qdata (
          G_OBJECT (pipeline), offerer_receives_video_quark ()))
      && GPOINTER_TO_INT (g_object_get_qdata (
          G_OBJECT (pipeline), answerer_receives_audio_quark ()))
      && GPOINTER_TO_INT (g_object_get_qdata (
          G_OBJECT (pipeline), answerer_receives_video_quark ()));
}
...
test_audio_video_sendonly_recvonly (const gchar *audio_enc_name,
    GstStaticCaps audio_expected_caps,
    gchar *audio_codec,
    const gchar *video_enc_name,
    GstStaticCaps video_expected_caps,
    gchar *video_codec,
    gboolean bundle)
{
  ...
  gst_bin_add (GST_BIN (pipeline), sender);
  connect_sink_async (
      sender, audiotestsrc, audio_enc, capsfilter, pipeline, SINK_AUDIO_STREAM);
  connect_sink_async (
      sender, videotestsrc, video_enc, NULL, pipeline, SINK_VIDEO_STREAM);
  ...
  g_signal_connect (
      receiver, "pad-added", G_CALLBACK (connect_sink_on_srcpad_added), NULL);
  fail_unless (
      kms_element_request_srcpad (receiver, KMS_ELEMENT_PAD_TYPE_AUDIO));
    ...
    gboolean bundle)
{
  ...
  g_object_set (
      G_OBJECT (audio_fakesink_offerer), "signal-handoffs", TRUE, NULL);
  ...
  g_object_set (
      G_OBJECT (video_fakesink_offerer), "signal-handoffs", TRUE, NULL);
  ...
  g_object_set (
      G_OBJECT (audio_fakesink_answerer), "signal-handoffs", TRUE, NULL);
  ...
  g_object_set (
      G_OBJECT (video_fakesink_answerer), "signal-handoffs", TRUE, NULL);
  ...
  g_object_set_qdata (
      G_OBJECT (offerer), audio_sink_quark (), audio_fakesink_offerer);
  fail_unless (
      kms_element_request_srcpad (offerer, KMS_ELEMENT_PAD_TYPE_AUDIO));
  g_object_set_qdata (
      G_OBJECT (answerer), audio_sink_quark (), audio_fakesink_answerer);
  fail_unless (
      kms_element_request_srcpad (answerer, KMS_ELEMENT_PAD_TYPE_AUDIO));
  ...
  g_object_set_qdata (
      G_OBJECT (offerer), video_sink_quark (), video_fakesink_offerer);
  fail_unless (
      kms_element_request_srcpad (offerer, KMS_ELEMENT_PAD_TYPE_VIDEO));
  g_object_set_qdata (
      G_OBJECT (answerer), video_sink_quark (), video_fakesink_answerer);
  fail_unless (
      kms_element_request_srcpad (answerer, KMS_ELEMENT_PAD_TYPE_VIDEO));
    ...
    gboolean bundle)
{
  ...
  g_object_set (offerer, "num-audio-medias", 1, "audio-codecs",
      g_array_ref (audio_codecs_array), "num-video-medias", 1, "video-codecs",
      g_array_ref (video_codecs_array), "bundle", bundle, "min-port", 50000,
      "max-port", 55000, NULL);
...
static void
webrtc_sender_pad_added (GstElement *element,
    GstPad *new_pad,
    gpointer user_data)
{
...
static void
webrtc_receiver_pad_added (GstElement *element,
    GstPad *new_pad,
    gpointer user_data)
{
  ...
  g_signal_connect (
      fakesink, "handoff", G_CALLBACK (fakesink_handoff), tmp->loop);
...
static void
data_session_established_cb (GstElement *self,
    const gchar *sess_id,
    gboolean connected,
    gpointer data)
{
  GST_DEBUG_OBJECT (
      self, "Data session %s", (connected) ? "established" : "finished");
...
test_data_channels (gboolean bundle)
{
  ...
  g_signal_connect (
      sender, "pad-added", G_CALLBACK (webrtc_sender_pad_added), pipeline);

  tmp.pipeline = pipeline;
  tmp.loop = loop;

  g_signal_connect (
      receiver, "pad-added", G_CALLBACK (webrtc_receiver_pad_added), &tmp);

  g_signal_emit_by_name (receiver, "request-new-pad", KMS_ELEMENT_PAD_TYPE_DATA,
      NULL, GST_PAD_SRC, &padname);
  ...
  g_signal_emit_by_name (
      sender, "get-local-sdp", sender_sess_id, &offerer_local_sdp);
  fail_unless (offerer_local_sdp != NULL);
  g_signal_emit_by_name (
      sender, "get-remote-sdp", sender_sess_id, &offerer_remote_sdp);
  fail_unless (offerer_remote_sdp != NULL);

  g_signal_emit_by_name (
      receiver, "get-local-sdp", receiver_sess_id, &answerer_local_sdp);
  fail_unless (answerer_local_sdp != NULL);
  g_signal_emit_by_name (
      receiver, "get-remote-sdp", receiver_sess_id, &answerer_remote_sdp);
  ...
  GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS (
      GST_BIN (pipeline), GST_DEBUG_GRAPH_SHOW_ALL, "test_data_channels_end");
...
{
  test_video_sendonly (
      "vp8enc", vp8_expected_caps, "VP8/90000", FALSE, FALSE, FALSE, NULL);
  test_video_sendonly (
      "vp8enc", vp8_expected_caps, "VP8/90000", TRUE, FALSE, FALSE, NULL);
  test_video_sendonly (
      "vp8enc", vp8_expected_caps, "VP8/90000", TRUE, TRUE, FALSE, NULL);
  test_video_sendonly (
      "vp8enc", vp8_expected_caps, "VP8/90000", TRUE, FALSE, TRUE, NULL);
}
GST_END_TEST
...
{
  test_video_sendonly (
      "vp8enc", vp8_expected_caps, "VP8/90000", FALSE, FALSE, FALSE, rsa_pem);
}
GST_END_TEST

const gchar *ecdsa_pem =
    "-----BEGIN EC PARAMETERS-----\r\n"
    "BggqhkjOPQMBBw==\r\n"
    "-----END EC PARAMETERS-----\r\n"
...
{
  test_video_sendonly (
      "vp8enc", vp8_expected_caps, "VP8/90000", FALSE, FALSE, FALSE, ecdsa_pem);
}
GST_END_TEST
...
static void
port_range_on_ice_candidate (GstElement *self,
    gchar *sess_id,
    KmsIceCandidate *candidate,
    CandidateRangeData *data)
{
  const guint port = kms_ice_candidate_get_port (candidate);

  GST_DEBUG ("Candidate: '%s'", kms_ice_candidate_get_candidate (candidate));
  GST_DEBUG ("Port: %u, min_port: %u, max_port: %u", port, data->min_port,
      data->max_port);
...
static void
not_enough_ports_on_ice_candidate (GstElement *self,
    gchar *sess_id,
    KmsIceCandidate *candidate,
...
static void
on_ice_candidate_check_mid (GstElement *self,
    gchar *sess_id,
    KmsIceCandidate *candidate,
    const gchar *expected_mid)
{
  const gchar *mid;
 ...
 */
static void
on_ice_candidate_check_ip (GstElement *self,
    gchar *sess_id,
    KmsIceCandidate *candidate,
    const gchar *expected_ip)
{
