  ...
  if (caps != NULL) {
    ...
    if (sync != NULL) {
      ...
      if (gst_structure_get_int (st, "clock-rate", &clock_rate)) {
        ...
        GST_ERROR_OBJECT (
            self, "Cannot get clock rate from caps: %" GST_PTR_FORMAT, caps);
      }
    }
...
static gboolean
kms_base_rtp_endpoint_sync_rtp_it (GstBuffer **buffer,
    guint idx,
...
static GstPadProbeReturn
kms_base_rtp_endpoint_sync_rtp_probe (GstPad *pad,
    GstPadProbeInfo *info,
    KmsRtpSynchronizer *sync)
{
  if (GST_PAD_PROBE_INFO_TYPE (info) & GST_PAD_PROBE_TYPE_BUFFER) {
...
static gboolean
kms_base_rtp_endpoint_sync_rtcp_it (GstBuffer **buffer,
    guint idx,
    KmsRtpSynchronizer *sync)
{
...
static GstPadProbeReturn
kms_base_rtp_endpoint_sync_rtcp_probe (GstPad *pad,
    GstPadProbeInfo *info,
    KmsRtpSynchronizer *sync)
{
  if (GST_PAD_PROBE_INFO_TYPE (info) & GST_PAD_PROBE_TYPE_BUFFER) {
