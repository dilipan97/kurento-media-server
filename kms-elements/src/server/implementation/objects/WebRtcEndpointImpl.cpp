...
remove_not_supported_codecs_from_array (GstElement *element, GArray *codecs)
{
  ...
  for (i = 0; i < codecs->len; i++) {
    ...
    if (!supported) {
      GST_INFO_OBJECT (
          element, "Removing not supported codec '%s'", codec_name);
...
WebRtcEndpointImpl::generateDefaultCertificates ()
{
  ...
  if (getConfigValue<std::string, WebRtcEndpoint> (
          &pemUriRSA, "pemCertificateRSA")) {
    defaultCertificateRSA = getCerficateFromFile (pemUriRSA);
  } else {
    std::string pemUri;
    if (getConfigValue<std::string, WebRtcEndpoint> (
            &pemUri, "pemCertificate")) {
      GST_WARNING (
          "pemCertificate is deprecated. Please use pemCertificateRSA instead");
      defaultCertificateRSA = getCerficateFromFile (pemUri);
    } else {
      GST_INFO (
          "Unable to load the RSA certificate from file. Using the default certificate.");
  ...
  if (getConfigValue<std::string, WebRtcEndpoint> (
          &pemUriECDSA, "pemCertificateECDSA")) {
    defaultCertificateECDSA = getCerficateFromFile (pemUriECDSA);
  } else {
    GST_INFO (
        "Unable to load the ECDSA certificate from file. Using the default certificate.");
    defaultCertificateECDSA = CertificateManager::generateECDSACertificate ();
  }
}

void
WebRtcEndpointImpl::checkUri (std::string &uri)
{
...
void
WebRtcEndpointImpl::onIceCandidate (gchar *sessId, KmsIceCandidate *candidate)
{
  ...
  std::shared_ptr<IceCandidate> cand (
      new IceCandidate (cand_str, mid_str, sdp_m_line_index));

  try {
    OnIceCandidate event (
        shared_from_this (), OnIceCandidate::getName (), cand);
    sigcSignalEmit (signalOnIceCandidate, event);
  } catch (const std::bad_weak_ptr &e) {
    // shared_from_this()
    GST_ERROR (
        "BUG creating %s: %s", OnIceCandidate::getName ().c_str (), e.what ());
  }

  try {
    IceCandidateFound event (
        shared_from_this (), IceCandidateFound::getName (), cand);
...
void
WebRtcEndpointImpl::onIceGatheringDone (gchar *sessId)
{
  try {
    OnIceGatheringDone event (
        shared_from_this (), OnIceGatheringDone::getName ());
...
void
WebRtcEndpointImpl::onIceComponentStateChanged (gchar *sessId,
    ...
    guint state)
{
  ...
  iceConnectionState.insert (
      std::pair<std::string, std::shared_ptr<IceConnection>> (
          key, connectionState));
...
void
WebRtcEndpointImpl::newSelectedPairFull (gchar *sessId,
    const gchar *streamId,
    guint componentId,
    KmsIceCandidate *localCandidate,
    KmsIceCandidate *remoteCandidate)
{
  ...
  GST_DEBUG_OBJECT (element,
      ...
      kms_ice_candidate_get_candidate (remoteCandidate), streamId, componentId);
  ...
  candidatePairs.insert (
      std::pair<std::string, std::shared_ptr<IceCandidatePair>> (
          key, candidatePair));
...
WebRtcEndpointImpl::onDataChannelOpened (gchar *sessId, guint stream_id)
{
  try {
    OnDataChannelOpened event (
        shared_from_this (), OnDataChannelOpened::getName (), stream_id);
  ...
  try {
    DataChannelOpen event (
        shared_from_this (), DataChannelOpen::getName (), stream_id);
    sigcSignalEmit (signalDataChannelOpen, event);
  } catch (const std::bad_weak_ptr &e) {
    // shared_from_this()
    GST_ERROR (
        "BUG creating %s: %s", DataChannelOpen::getName ().c_str (), e.what ());
  }
}
...
WebRtcEndpointImpl::onDataChannelClosed (gchar *sessId, guint stream_id)
{
  try {
    OnDataChannelClosed event (
        shared_from_this (), OnDataChannelClosed::getName (), stream_id);
  ...
  try {
    DataChannelClose event (
        shared_from_this (), DataChannelClose::getName (), stream_id);
...
void
WebRtcEndpointImpl::postConstructor ()
{
  BaseRtpEndpointImpl::postConstructor ();

  handlerOnIceCandidate =
      register_signal_handler (G_OBJECT (element), "on-ice-candidate",
          std::function<void (GstElement *, gchar *, KmsIceCandidate *)> (
              std::bind (&WebRtcEndpointImpl::onIceCandidate, this,
                  std::placeholders::_2, std::placeholders::_3)),
          std::dynamic_pointer_cast<WebRtcEndpointImpl> (shared_from_this ()));

  handlerOnIceGatheringDone =
      register_signal_handler (G_OBJECT (element), "on-ice-gathering-done",
          std::function<void (GstElement *, gchar *)> (
              std::bind (&WebRtcEndpointImpl::onIceGatheringDone, this,
                  std::placeholders::_2)),
          std::dynamic_pointer_cast<WebRtcEndpointImpl> (shared_from_this ()));

  handlerOnIceComponentStateChanged = register_signal_handler (
      G_OBJECT (element), "on-ice-component-state-changed",
  ...
  handlerNewSelectedPairFull =
      register_signal_handler (G_OBJECT (element), "new-selected-pair-full",
          std::function<void (GstElement *, gchar *, gchar *, guint,
  ...
  handlerOnDataChannelOpened =
      register_signal_handler (G_OBJECT (element), "data-channel-opened",
          std::function<void (GstElement *, gchar *, guint)> (
              std::bind (&WebRtcEndpointImpl::onDataChannelOpened, this,
                  std::placeholders::_2, std::placeholders::_3)),
          std::dynamic_pointer_cast<WebRtcEndpointImpl> (shared_from_this ()));

  handlerOnDataChannelClosed =
      register_signal_handler (G_OBJECT (element), "data-channel-closed",
          std::function<void (GstElement *, gchar *, guint)> (
        ...
        FACTORY_NAME)
{
  ...
  if (getConfigValue<std::string, WebRtcEndpoint> (
          &externalIPv4, PARAM_EXTERNAL_IPV4)) {
    GST_INFO ("Predefined external IPv4 address: %s", externalIPv4.c_str ());
    g_object_set (
        G_OBJECT (element), PROP_EXTERNAL_IPV4, externalIPv4.c_str (), NULL);
  } else {
  ...
  if (getConfigValue<std::string, WebRtcEndpoint> (
          &externalIPv6, PARAM_EXTERNAL_IPV6)) {
    GST_INFO ("Predefined external IPv6 address: %s", externalIPv6.c_str ());
    g_object_set (
        G_OBJECT (element), PROP_EXTERNAL_IPV6, externalIPv6.c_str (), NULL);
  } else {
  ...
  if (getConfigValue<std::string, WebRtcEndpoint> (
          &externalAddress, PARAM_EXTERNAL_ADDRESS)) {
  ...
  if (getConfigValue<std::string, WebRtcEndpoint> (
          &networkInterfaces, PARAM_NETWORK_INTERFACES)) {
  ...
  if (getConfigValue<gboolean, WebRtcEndpoint> (&iceTcp, PARAM_ICE_TCP)) {
    GST_INFO (
        "ICE-TCP candidate gathering is %s", iceTcp ? "ENABLED" : "DISABLED");
  ...
  if (!getConfigValue<uint, WebRtcEndpoint> (
          &stunPort, "stunServerPort", DEFAULT_STUN_PORT)) {
  ...
  case CertificateKeyType::RSA: {
    if (defaultCertificateRSA != "") {
      g_object_set (G_OBJECT (element), "pem-certificate",
          defaultCertificateRSA.c_str (), NULL);
    }
  ...
  case CertificateKeyType::ECDSA: {
    if (defaultCertificateECDSA != "") {
      g_object_set (G_OBJECT (element), "pem-certificate",
          defaultCertificateECDSA.c_str (), NULL);
    }
...
{
  g_object_set (
      G_OBJECT (element), "stun-server", stunServerAddress.c_str (), NULL);
}
...
WebRtcEndpointImpl::getIceConnectionState ()
{
  ...
  for (it = iceConnectionState.begin (); it != iceConnectionState.end ();
       it++) {
...
WebRtcEndpointImpl::gatherCandidates ()
{
  ...
  g_signal_emit_by_name (
      element, "gather-candidates", this->sessId.c_str (), &ret);

  if (!ret) {
    throw KurentoException (
        ICE_GATHER_CANDIDATES_ERROR, "Error gathering candidates");
  }
}
...
WebRtcEndpointImpl::addIceCandidate (std::shared_ptr<IceCandidate> candidate)
{
  ...
  if (cand) {
    g_signal_emit_by_name (
        element, "add-ice-candidate", this->sessId.c_str (), cand, &ret);
    g_object_unref (cand);

    if (!ret) {
      throw KurentoException (
          ICE_ADD_CANDIDATE_ERROR, "Error adding candidate");
    }
  }
}
    ...
    const std::string &protocol)
{
  ...
  g_signal_emit_by_name (
      element, "get-data-channel-supported", this->sessId.c_str (), &supported);
  ...
  if (maxPacketLifeTime > G_MAXUSHORT) {
    GST_WARNING (
        "maxPacketLifeTime can not be bigger than %u. Setting it to that value",
        G_MAXUSHORT);
  ...
  if (maxRetransmits > G_MAXUSHORT) {
    GST_WARNING (
        "maxRetransmits can not be bigger than %u. Setting it to that value",
        G_MAXUSHORT);
  ...
  g_signal_emit_by_name (element, "create-data-channel", this->sessId.c_str (),
      ordered, lifeTime, retransmits, label.c_str (), protocol.c_str (),
      &stream_id);
...
WebRtcEndpointImpl::closeDataChannel (int channelId)
{
  ...
  g_signal_emit_by_name (
      element, "destroy-data-channel", this->sessId.c_str (), channelId);
}
...
getRTCDataChannelState (KmsWebRtcDataChannelState state)
{
  ...
  case KMS_WEB_RTC_DATA_CHANNEL_STATE_CONNECTING:
    rtcDataChannelState =
        std::make_shared<RTCDataChannelState> (RTCDataChannelState::connecting);
    break;

  case KMS_WEB_RTC_DATA_CHANNEL_STATE_OPEN:
    rtcDataChannelState =
        std::make_shared<RTCDataChannelState> (RTCDataChannelState::open);
    break;

  case KMS_WEB_RTC_DATA_CHANNEL_STATE_CLOSING:
    rtcDataChannelState =
        std::make_shared<RTCDataChannelState> (RTCDataChannelState::closing);
    break;

  case KMS_WEB_RTC_DATA_CHANNEL_STATE_CLOSED:
    rtcDataChannelState =
        std::make_shared<RTCDataChannelState> (RTCDataChannelState::closed);
    break;
  }
...
createtRTCDataChannelStats (const GstStructure *stats)
{
  ...
  gst_structure_get (stats, "channel-id", G_TYPE_UINT, &channelid, "label",
      ...
      G_TYPE_UINT64, &bytes_sent, "bytes-recv", G_TYPE_UINT64, &bytes_recv,
      "messages-sent", G_TYPE_UINT64, &messages_sent, "messages-recv",
      G_TYPE_UINT64, &message_recv, NULL);
...
createtRTCPeerConnectionStats (const GstStructure *stats)
{
  ...
  gst_structure_get (stats, "data-channels-opened", G_TYPE_UINT, &opened,
      "data-channels-closed", G_TYPE_UINT, &closed, "id", G_TYPE_STRING, &id,
      NULL);

  std::shared_ptr<RTCPeerConnectionStats> peerConnStats =
      std::make_shared<RTCPeerConnectionStats> (id,
          std::make_shared<StatsType> (StatsType::session), 0.0, 0, opened,
          closed);
  g_free (id);
...
void
WebRtcEndpointImpl::fillStatsReport (
    ...
    int64_t timestampMillis)
{
  ...
  BaseRtpEndpointImpl::fillStatsReport (
      report, stats, timestamp, timestampMillis);

  data_stats = kms_utils_get_structure_by_name (
      stats, KMS_DATA_SESSION_STATISTICS_FIELD);

  if (data_stats != nullptr) {
    return collectRTCDataChannelStats (
        report, timestamp, timestampMillis, data_stats);
  }
}

MediaObjectImpl *
WebRtcEndpointImplFactory::createObject (
...
{
  return new WebRtcEndpointImpl (conf, mediaPipeline, recvonly, sendonly,
      useDataChannels, certificateKeyType);
}
