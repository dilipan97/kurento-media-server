...
#define KMS_AGNOSTIC_BIN2_GET_PRIVATE(obj)                                     \
  (G_TYPE_INSTANCE_GET_PRIVATE (                                               \
      (obj), KMS_TYPE_AGNOSTIC_BIN2, KmsAgnosticBin2Private))
...
enum { SIGNAL_MEDIA_TRANSCODING, LAST_SIGNAL };
...
static GstBin *kms_agnostic_bin2_find_or_create_bin_for_caps (
...
{
  g_hash_table_insert (
      self->priv->bins, GST_OBJECT_NAME (bin), g_object_ref (bin));
}
...
send_dummy_event (GstPad *pad)
{
  ...
  if (GST_PAD_IS_SINK (pad)) {
    ...
    gst_pad_send_event (pad,
        gst_event_new_custom (
            GST_EVENT_TYPE_UPSTREAM | GST_EVENT_TYPE_SERIALIZED,
            gst_structure_new_from_string ("dummy")));
  }
...
remove_on_unlinked_async (gpointer data, gpointer not_used)
{
  ...
  if (g_strcmp0 (GST_OBJECT_NAME (gst_element_get_factory (elem)), "queue")
      == 0) {
...
remove_on_unlinked_blocked (GstPad *pad, GstPadProbeInfo *info, gpointer elem)
{
  ...
  g_object_set_qdata (
      G_OBJECT (pad), unlinking_data_quark (), GINT_TO_POINTER (TRUE));
...
static void
remove_element_on_unlinked (GstElement *element,
    ...
    gchar *sink_name)
{
  ...
  g_signal_connect (
      pad, "unlinked", G_CALLBACK (remove_on_unlinked_cb), sink_name);
...
link_element_to_tee (GstElement *tee, GstElement *element)
{
  ...
  g_signal_connect (
      tee_src, "unlinked", G_CALLBACK (remove_tee_pad_on_unlink), NULL);

  gst_pad_add_probe (
      tee_src, GST_PAD_PROBE_TYPE_EVENT_UPSTREAM, tee_src_probe, NULL, NULL);
...
static gboolean
proxy_src_pad_query_function (GstPad *pad, GstObject *parent, GstQuery *query)
{
...
static void
kms_agnostic_bin2_link_to_tee (KmsAgnosticBin2 *self,
    GstPad *pad,
    GstElement *tee,
    GstCaps *caps)
{
  ...
  gst_pad_set_query_function (
      GST_PAD_CAST (proxy), proxy_src_pad_query_function);
...
check_bin (KmsTreeBin *tree_bin, const GstCaps *caps)
{
  ...
  GST_INFO_OBJECT (tree_bin,
      "TreeBin '%" GST_PTR_FORMAT "' caps: %" GST_PTR_FORMAT, tree_bin,
      current_caps);
  ...
  if (current_caps != NULL && gst_caps_get_size (current_caps) > 0) {
    ...
    gst_caps_set_features (
        caps_without_features, 0, gst_caps_features_new_empty ());
...
kms_agnostic_bin2_get_or_create_dec_bin (KmsAgnosticBin2 *self, GstCaps *caps)
{
  ...
  if (kms_utils_caps_is_raw (self->priv->input_caps) || gst_caps_is_empty (caps)
    ...
    GstCaps *caps)
{
  ...
  GST_INFO_OBJECT (
      self, "SIGNAL_MEDIA_TRANSCODING for %d", SIGNAL_MEDIA_TRANSCODING);
  if (kms_utils_caps_is_audio (caps)) {
  ...
  GST_INFO_OBJECT (
      self, "Find TreeBin with wanted caps: %" GST_PTR_FORMAT, caps);

  bin = kms_agnostic_bin2_find_bin_for_caps (self, caps);

  if (bin == NULL) {
    GST_LOG_OBJECT (
        self, "TreeBin not found! Transcoding required for %s", media_type);
    ...
    if (!self->priv->transcoding_emitted) {
      ...
      GST_LOG_OBJECT (
          self, "Suppressed - TRANSCODING ACTIVE for %s", media_type);
    }
  } else {
    ...
    if (!self->priv->transcoding_emitted) {
      ...
      GST_LOG_OBJECT (
          self, "Suppressed - TRANSCODING INACTIVE for %s", media_type);
    }
  }
...
kms_agnostic_bin2_link_pad (KmsAgnosticBin2 *self, GstPad *pad, GstPad *peer)
{
  ...
  GST_INFO_OBJECT (
      self, "Linking: %" GST_PTR_FORMAT " to %" GST_PTR_FORMAT, pad, peer);

  pad_caps = gst_pad_query_caps (pad, NULL);
  if (pad_caps != NULL) {
    GST_DEBUG_OBJECT (
        self, "Upstream provided caps: %" GST_PTR_FORMAT, pad_caps);
  ...
  GST_DEBUG_OBJECT (
      self, "Downstream wanted caps: %" GST_PTR_FORMAT, peer_caps);
...
kms_agnostic_bin2_process_pad (KmsAgnosticBin2 *self, GstPad *pad)
{
  ...
  GST_INFO_OBJECT (self, "Processing pad: %" GST_PTR_FORMAT, pad);
  ...
  if (peer != NULL) {
    ...
    if (target) {
      ...
      if (caps != NULL) {
        ...
        if (accepted) {
          GST_TRACE_OBJECT (
              self, "No need to reconfigure pad %" GST_PTR_FORMAT, pad);
          g_object_unref (target);
          g_object_unref (peer);
          return FALSE;
        }
...
input_bin_src_caps_probe (GstPad *pad, GstPadProbeInfo *info, gpointer bin)
{
  ...
  kms_element_for_each_src_pad (
      GST_ELEMENT (self), (KmsPadIterationAction)add_linked_pads, self);
...
kms_agnostic_bin2_configure_input (KmsAgnosticBin2 *self, const GstCaps *caps)
{
  ...
  if (self->priv->input_bin != NULL) {
    kms_tree_bin_unlink_input_element_from_tee (
        KMS_TREE_BIN (self->priv->input_bin));
  }
...
static GstPadProbeReturn
kms_agnostic_bin2_sink_caps_probe (GstPad *pad,
    ...
    gpointer user_data)
{
  ...
  GST_TRACE_OBJECT (
      pad, "Self: %s, event: %" GST_PTR_FORMAT, GST_ELEMENT_NAME (self), event);
  ...
  if (current_caps != NULL) {
    ...
    gst_structure_remove_fields (
        st, "width", "height", "framerate", "streamheader", "codec_data", NULL);
...
static GstPadProbeReturn
kms_agnostic_bin2_src_reconfigure_probe (GstPad *pad,
    GstPadProbeInfo *info,
    gpointer user_data)
{
...
static void
kms_agnostic_bin2_src_unlinked (GstPad *pad,
    GstPad *peer,
    KmsAgnosticBin2 *self)
{
    ...
    const GstCaps *caps)
{
  ...
  g_signal_connect (
      pad, "unlinked", G_CALLBACK (kms_agnostic_bin2_src_unlinked), self);
...
void
kms_agnostic_bin2_set_property (GObject *object,
    guint property_id,
    const GValue *value,
    GParamSpec *pspec)
{
  ...
  case PROP_MIN_BITRATE: {
    ...
    if (v > self->priv->max_bitrate) {
      ...
      GST_WARNING_OBJECT (self, "Setting min-bitrate bigger than max-bitrate");
    }

    self->priv->min_bitrate = v;
    GST_LOG_OBJECT (self, "min_bitrate configured %d", self->priv->min_bitrate);
...
void
kms_agnostic_bin2_get_property (GObject *object,
    guint property_id,
    GValue *value,
    GParamSpec *pspec)
{
...
kms_agnostic_bin2_class_init (KmsAgnosticBin2Class *klass)
{
  ...
  gst_element_class_set_details_simple (gstelement_class,
      "Agnostic connector 2nd version", "Generic/Bin/Connector",
  ...
  g_object_class_install_property (gobject_class, PROP_MIN_BITRATE,
      g_param_spec_int ("min-bitrate", "min bitrate",
          "Configure the min bitrate to media encoding", 0, G_MAXINT,
          MIN_BITRATE_DEFAULT, G_PARAM_READWRITE));

  g_object_class_install_property (gobject_class, PROP_MAX_BITRATE,
      g_param_spec_int ("max-bitrate", "max bitrate",
          "Configure the max bitrate to media encoding", 0, G_MAXINT,
          MAX_BITRATE_DEFAULT, G_PARAM_READWRITE));

  g_object_class_install_property (gobject_class, PROP_CODEC_CONFIG,
      g_param_spec_boxed ("codec-config", "codec config", "Codec configuration",
   ...
   */
  kms_agnostic_bin2_signals[SIGNAL_MEDIA_TRANSCODING] = g_signal_new (
      "media-transcoding", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET (KmsAgnosticBin2Class, media_transcoding), NULL, NULL,
      NULL, G_TYPE_NONE, 2, G_TYPE_BOOLEAN, KMS_TYPE_MEDIA_TYPE);

  g_type_class_add_private (klass, sizeof (KmsAgnosticBin2Private));
}

static gboolean
kms_agnostic_bin2_sink_query (GstPad *pad, GstObject *parent, GstQuery *query)
{
  gboolean ret;
...
check_ret_error (GstPad *pad, GstFlowReturn ret)
{
  ...
  case GST_FLOW_ERROR: {
    ...
    g_object_get (
        self->priv->input_fakesink, "last-message", &fakesink_message, NULL);
    GST_FIXME_OBJECT (
        pad, "Handling flow error, fakesink message: %s", fakesink_message);
    ...
    g_object_set (
        fakesink, "async", FALSE, "sync", FALSE, "silent", FALSE, NULL);
  ...
  case GST_FLOW_NOT_LINKED:
    // TODO: We should notify this as an error to remote client
    GST_WARNING_OBJECT (
        pad, "Ignoring flow status: %s", gst_flow_get_name (ret));
    ret = GST_FLOW_OK;
    break;
  default:
...
static GstFlowReturn
kms_agnostic_bin2_sink_chain (GstPad *pad, GstObject *parent, GstBuffer *buffer)
{
  GstFlowReturn ret;
...
kms_agnostic_bin2_init (KmsAgnosticBin2 *self)
{
  ...
  fakesink = kms_utils_element_factory_make ("fakesink", "agnosticbin");
  self->priv->input_fakesink = fakesink;
  g_object_set (fakesink, "async", FALSE, "sync", FALSE, "silent",
      FALSE, // FIXME used to print log in check_ret_error()
      NULL);
  ...
  gst_pad_set_chain_list_function (
      self->priv->sink, kms_agnostic_bin2_sink_chain_list);
...
kms_agnostic_bin2_plugin_init (GstPlugin *plugin)
{
  return gst_element_register (
      plugin, PLUGIN_NAME, GST_RANK_NONE, KMS_TYPE_AGNOSTIC_BIN2);
}
