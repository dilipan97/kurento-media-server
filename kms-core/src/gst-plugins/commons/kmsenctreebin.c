...
#define KMS_ENC_TREE_BIN_GET_PRIVATE(obj)                                      \
  (G_TYPE_INSTANCE_GET_PRIVATE (                                               \
      (obj), KMS_TYPE_ENC_TREE_BIN, KmsEncTreeBinPrivate))

#define KMS_ENC_TREE_BIN_LIMIT(obj, value)                                     \
  MAX ((obj)->priv->min_bitrate, MIN ((obj)->priv->max_bitrate, (value)))

typedef enum { VP8, X264, OPENH264, OPUS, UNSUPPORTED } EncoderType;
...
static void
set_encoder_configuration (GstElement *encoder,
    ...
    const gchar *config_name)
{
  ...
  if (gst_structure_has_field_typed (
          codec_config, config_name, GST_TYPE_STRUCTURE)) {
    GstStructure *config;
    guint n_props = 0, i;
    GParamSpec **props;

    gst_structure_get (
        codec_config, config_name, GST_TYPE_STRUCTURE, &config, NULL);
...
static void
configure_encoder (GstElement *encoder,
    EncoderType type,
    gint target_bitrate,
    GstStructure *codec_configs)
{
  ...
  case VP8: {
    ...
    g_object_set (G_OBJECT (encoder), "deadline", G_GINT64_CONSTANT (200000),
        "threads", 1, "cpu-used", 16, "resize-allowed", TRUE, "target-bitrate",
        target_bitrate, "end-usage", /* cbr */ 1, NULL);
    /* *INDENT-ON* */
    break;
  }
  case X264: {
    /* *INDENT-OFF* */
    g_object_set (G_OBJECT (encoder), "speed-preset", /* veryfast */ 3,
        "threads", (guint)1, "bitrate", target_bitrate / 1000, "key-int-max",
        60, "tune", /* zero-latency */ 4, NULL);
    /* *INDENT-ON* */
    break;
  }
  case OPENH264: {
    /* *INDENT-OFF* */
    g_object_set (G_OBJECT (encoder), "rate-control", /* bitrate */ 1,
        "bitrate", target_bitrate, NULL);
    /* *INDENT-ON* */
    break;
  }
  case OPUS: {
    g_object_set (G_OBJECT (encoder), "inband-fec", TRUE, "perfect-timestamp",
        TRUE, NULL);
    break;
  }
  default:
    GST_DEBUG ("Codec %" GST_PTR_FORMAT
               " not configured because it is not supported",
        encoder);
    break;
  }
  set_encoder_configuration (
      encoder, codec_configs, kms_enc_tree_bin_get_name_from_type (type));
}
    ...
    GstStructure *codec_configs)
{
  ...
  encoder_list = gst_element_factory_list_get_elements (
  ...
  if (encoder_factory != NULL) {
    ...
    configure_encoder (
        self->priv->enc, self->priv->enc_type, target_bitrate, codec_configs);
  }
...
kms_enc_tree_bin_get_bitrate (KmsEncTreeBin *self)
{
  ...
  if (self->priv->remb_bitrate <= 0) {
    ...
    bitrate = KMS_ENC_TREE_BIN_LIMIT (
        self, MIN (self->priv->remb_bitrate, self->priv->tag_bitrate));
  }
...
kms_enc_tree_bin_set_target_bitrate (KmsEncTreeBin *self)
{
  ...
  GST_DEBUG_OBJECT (
      self->priv->enc, "Set target encoding bitrate: %d bps", target_bitrate);
...
void
kms_enc_tree_bin_set_bitrate_limits (KmsEncTreeBin *self,
    gint min_bitrate,
    gint max_bitrate)
{
...
static void
bitrate_callback (RembEventManager *remb_manager,
    guint bitrate,
    gpointer user_data)
{
...
static gboolean
kms_enc_tree_bin_configure (KmsEncTreeBin *self,
    const GstCaps *caps,
    gint target_bitrate,
    GstStructure *codec_configs)
{
  ...
  kms_enc_tree_bin_create_encoder_for_caps (
      self, caps, target_bitrate, codec_configs);

  if (self->priv->enc == NULL) {
    GST_WARNING_OBJECT (
        self, "Invalid encoder for caps: %" GST_PTR_FORMAT, caps);
    return FALSE;
  }
  ...
  kms_utils_remb_event_manager_set_callback (
      self->priv->remb_manager, bitrate_callback, self, NULL);
  ...
  gst_bin_add_many (
      GST_BIN (self), convert, mediator, queue, self->priv->enc, NULL);
  ...
  if (self->priv->enc_type == X264) {
    ...
    gst_element_link_many (
        convert, mediator, queue, self->priv->enc, output_tee, NULL);
  }
...
kms_enc_tree_bin_class_init (KmsEncTreeBinClass *klass)
{
  ...
  gst_element_class_set_details_simple (gstelement_class, "EncTreeBin",
      "Generic", "Bin to encode and distribute encoding media.",
