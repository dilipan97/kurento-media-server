...
PlayerEndpointImpl::eosHandler ()
{
  try {
    ...
    // shared_from_this()
    GST_ERROR (
        "BUG creating %s: %s", EndOfStream::getName ().c_str (), e.what ());
  }
}

void
PlayerEndpointImpl::invalidUri ()
{
...
void
PlayerEndpointImpl::invalidMedia ()
{
  /* TODO: Define error codes and types*/
  try {
...
void
PlayerEndpointImpl::postConstructor ()
{
  ...
  signalEOS = register_signal_handler (G_OBJECT (element), "eos",
      ...
      std::dynamic_pointer_cast<PlayerEndpointImpl> (shared_from_this ()));
  ...
  signalInvalidMedia =
      register_signal_handler (G_OBJECT (element), "invalid-media",
          std::function<void (GstElement *)> (
        ...
        uri)
{
  ...
  if (getConfigValue<std::string, PlayerEndpoint> (
          &portRange, RTSP_CLIENT_PORT_RANGE)) {
...
std::shared_ptr<VideoInfo>
PlayerEndpointImpl::getVideoInfo ()
{
  ...
  gst_structure_get (video_data, "isSeekable", G_TYPE_BOOLEAN, &isSeekable,
      "seekableInit", G_TYPE_INT64, &seekableInit, "seekableEnd", G_TYPE_INT64,
      &seekableEnd, "duration", G_TYPE_INT64, &duration, NULL);

  videoInfo = std::make_shared<VideoInfo> (VideoInfo (isSeekable,
      seekableInit / NS_TO_MS, seekableEnd / NS_TO_MS, duration / NS_TO_MS));
  return videoInfo;
}

int64_t
PlayerEndpointImpl::getPosition ()
{
  int64_t position;
...
std::string
PlayerEndpointImpl::getElementGstreamerDot ()
{
  GValue *pipeline;
...
void
PlayerEndpointImpl::setPosition (int64_t position)
{
  gboolean ret;
...
MediaObjectImpl *
PlayerEndpointImplFactory::createObject (
    ...
    int networkCache) const
{
  return new PlayerEndpointImpl (
      conf, mediaPipeline, uri, useEncodedMedia, networkCache);
}
